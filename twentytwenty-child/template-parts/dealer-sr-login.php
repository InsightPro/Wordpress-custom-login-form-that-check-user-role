	...
	<main id="site-content" role="main" class="dealer--login-page">
	  ...
	  <div class="wp-block-columns alignwide">
	    <div id="dealer--login" class="wp-block-column dealer--login">
	      ...
	      <?php //echo do_shortcode( '[gravityform action="login" title="false" description="false" login_redirect= site_url()."/dealer-login/" registration_link_display="false"/]' ); ?>
				<?php wp_login_form( );?>
	      <?php
					//echo do_shortcode( '[gravityform action="login" title="false" description="false" login_redirect=site_url()."/sr-dealer-login/" registration_link_display="false"/]' );
					global $wp;
			 		$user = wp_get_current_user(); 
			 		$user_role = $user->roles[0];
					$redirect_to = site_url( '/dealer-login' );
					$get_user_by_email = $wpdb->get_results( $wpdb->prepare( "SELECT ID FROM {$user_table} WHERE user_email = '$user_email' ") );
			    $user_id = $get_user_by_email[0]->ID;
			    ...
			    $userrole  = $user_roles[0];
					if ($user_role != NULL) {
						if ($user_role == 'dealer' || $user_role == 'sales_r') {
							...
							wp_logout();
							$url = site_url(  '/dealer-login' );
							wp_redirect($url.'?dealer_login_error');
						}
					}
			  ?>
	    </div>
